...
from data_preprocessing import data_loader, cls_data_loader, train_data_loader, test_data_loader
from metric import accuracy, sensitivity, specificity, get_clf_eval
from model import GAT, GCN, ChebyNet, GCNR
...
def train(model, optimizer, criterion, train_loader, epoch, num_epochs):
    ...
    for batch_idx, loader in enumerate(train_loader):
        ...
        lambda_ = get_lambda(epoch, num_epochs)
...
def trainer(args, seed, train_x, test_x, train_y, train_g_y, test_y):
    ...
    # define model
    model = GCNR(args).to(args.device)  #  GAT, GCN, ChebNet
    wandb.watch(model)
    ...
    train_loader, test_loader = cls_data_loader(args, train_x, train_y, train_g_y, test_x, test_y)
    ...
    for epoch in tqdm(range(1, args.num_epochs + 1)):
        train_loss, train_acc, train_sen, train_spe, train_f1 = train(model, optimizer, criterion, train_loader, epoch, args.num_epochs)
...
def trainer(args, seed, train_loader, test_loader):
...
def main(seed, timestamp):
    ...
    wandb.init(project="ASD_classification", name=f"NYU_multi_site_group_{args.timestamp}")
    ...
    PITT_site_dataset = train_data_loader(args, PITT_site_data, PITT_site_label, PITT_site_group_label)
    UM_1_site_dataset = train_data_loader(args, UM_1_site_data, UM_1_site_label, UM_1_site_group_label)
    USM_site_dataset = train_data_loader(args, USM_site_data, USM_site_label, USM_site_group_label)
    YALE_site_dataset = train_data_loader(args, YALE_site_data, YALE_site_label, YALE_site_group_label)
    UCLA_1_site_dataset = train_data_loader(args, UCLA_1_site_data, UCLA_1_site_label, UCLA_1_site_group_label)
    NYU_train_dataset = train_data_loader(args, NYU_train, NYU_train_label, NYU_site_group_label)
    NYU_test_dataset = test_data_loader(args, NYU_test, NYU_test_label)

    train_dataset = sum([PITT_site_dataset, UM_1_site_dataset, USM_site_dataset, YALE_site_dataset, UCLA_1_site_dataset, NYU_train_dataset], [])
    test_dataLoader = NYU_test_dataset
    ...
    test_loss, test_acc, test_sen, test_spe, test_f1 = trainer(args, seed, trainLoader, test_dataLoader)
