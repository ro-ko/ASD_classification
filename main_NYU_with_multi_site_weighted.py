...
from data_preprocessing import data_loader, group_data_loader
...
def train(model, optimizer, criterion, train_loader, site_weight_dict):
    model.train()
    train_epoch_cost = 0
    ...
    for batch_idx, loader in enumerate(train_loader):
        ...
        batch_weights = torch.tensor([site_weight_dict[int(g.item())] for g in group_label],
                                       dtype=torch.float, device=outputs.device)  # shape: [batch_size]
        weighted_loss = (loss * batch_weights).mean()

        scale_factor = 5.0  # 예시 값, 필요에 따라 조정
        scaled_weighted_loss = weighted_loss * scale_factor

        scaled_weighted_loss.backward()
...
def trainer(args, seed, train_x, test_x, train_y, train_g_y, test_y, site_weight_dict):
    ...
    train_loader, test_loader = group_data_loader(args, train_x, train_y, train_g_y, test_x, test_y)
    ...
    for epoch in tqdm(range(1, args.num_epochs + 1)):
        train_loss, train_acc, train_sen, train_spe, train_f1 = train(model, optimizer, criterion, train_loader, site_weight_dict)
...
def main(seed, timestamp):
    ...
    site_counts = [PITT_site_data.shape[0], UM_1_site_data.shape[0], USM_site_data.shape[0], YALE_site_data.shape[0], UCLA_1_site_data.shape[0], NYU_train.shape[0]]
    all_data_num = sum(site_counts)
    site_weight_dict = {i : site_counts[i] / all_data_num for i in range(len(site_counts))}
    ...
    test_loss, test_acc, test_sen, test_spe, test_f1 = trainer(args, seed, train_x, NYU_test, train_label, train_group, NYU_test_label, site_weight_dict)
